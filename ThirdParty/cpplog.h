...
//          Tries to use syscall(SYS_gettid) to get the current Thread ID.  This
//          is especially useful on systems where boost's
//          get_current_thread_id() will return a structure (usually pthread_t),
//          which isn't incredibly useful. NOTE: Only useful if you also #define
//          CPPLOG_SYSTEM_IDS
//
...
// #define CPPLOG_FILTER_LEVEL               WARN
// #define CPPLOG_SYSTEM_IDS
// #define CPPLOG_USE_SYSCALL_FOR_THREAD_ID
// #define CPPLOG_THREADING
#define CPPLOG_HELPER_MACROS
...
#define CPPLOG_FILTER_LEVEL DEBUG
#endif
...
//      - LogMessage's constructor captures __FILE__, __LINE__, severity and our
//      output logger.
//      - LogMessage exposes a function getStream(), which is an
//      ostringstream-style stream that
//        client code can write debug information into.
//      - When the sendToLogger() method of a LogMessage is called, all the
//      buffered data in the
//        messages' stream is sent to the specified logger.
//      - When a LogMessage's destructor is called, it calls sendToLogger() to
//      send all
//        remaining data.
