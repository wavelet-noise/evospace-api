...
template <class T> [[nodiscard]] static constexpr auto typeName() noexcept {
    constexpr std::string_view prettyName{LUABRIDGE_PRETTY_FUNCTION};

    constexpr auto first = prettyName.find_first_not_of(
        ' ', prettyName.find_first_of(LUABRIDGE_PRETTY_FUNCTION_PREFIX) + 1
    );

    return prettyName.substr(
        first, prettyName.find_last_of(LUABRIDGE_PRETTY_FUNCTION_SUFFIX) - first
    );
}
 ...
 *
 * The static table holds the static data members, static properties, and static
 * member functions for a class.
 */
template <class T> [[nodiscard]] const void *getStaticRegistryKey() noexcept {
 ...
 *
 * The class table holds the data members, properties, and member functions of a
 * class. Read-only data and properties, and const member functions are also
 * placed here (to save a lookup in the const table).
 */
template <class T> [[nodiscard]] const void *getClassRegistryKey() noexcept {
 ...
 *
 * The const table holds read-only data members and properties, and const member
 * functions of a class.
 */
template <class T> [[nodiscard]] const void *getConstRegistryKey() noexcept {
