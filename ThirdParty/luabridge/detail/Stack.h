...
class StackRestore final {
  public:
    StackRestore(lua_State *L) : m_L(L), m_stackTop(lua_gettop(L)) {}
    ...
    void reset() { m_doRestoreStack = false; }
 ...
 * @tparam T A C++ type.
 */
template <class T, class> struct Stack;
...
template <> struct Stack<void> {
    [[nodiscard]] static Result push(lua_State *) { return {}; }
};
 ...
 */
template <> struct Stack<std::nullptr_t> {
    ...
    [[nodiscard]] static TypeResult<std::nullptr_t>
 ...
 */
template <> struct Stack<lua_State *> {
 ...
 */
template <> struct Stack<lua_CFunction> {
    ...
    [[nodiscard]] static TypeResult<lua_CFunction>
    get(lua_State *L, int index) {
 ...
 */
template <> struct Stack<std::byte> {
    ...
    [[nodiscard]] static Result push(lua_State *L, std::byte value) {
        ...
        pushunsigned(
            L, std::to_integer<std::make_unsigned_t<lua_Integer>>(value)
        );
        return {};
    }
 ...
 */
template <> struct Stack<unsigned char> {
    ...
    [[nodiscard]] static TypeResult<unsigned char>
    get(lua_State *L, int index) {
        if (lua_type(L, index) != LUA_TNUMBER)
            return makeErrorCode(ErrorCode::InvalidTypeCast);
 ...
 */
template <> struct Stack<unsigned short> {
    ...
    [[nodiscard]] static TypeResult<unsigned short>
    get(lua_State *L, int index) {
        if (lua_type(L, index) != LUA_TNUMBER)
            return makeErrorCode(ErrorCode::InvalidTypeCast);
 ...
 */
template <> struct Stack<unsigned int> {
 ...
 */
template <> struct Stack<unsigned long> {
    ...
    [[nodiscard]] static TypeResult<unsigned long>
    get(lua_State *L, int index) {
        if (lua_type(L, index) != LUA_TNUMBER)
            return makeErrorCode(ErrorCode::InvalidTypeCast);
 ...
 */
template <> struct Stack<unsigned long long> {
    ...
    [[nodiscard]] static TypeResult<unsigned long long>
    get(lua_State *L, int index) {
        if (lua_type(L, index) != LUA_TNUMBER)
            return makeErrorCode(ErrorCode::InvalidTypeCast);
 ...
 */
template <> struct Stack<long double> {
 ...
 */
template <> struct Stack<std::string_view> {
    ...
    [[nodiscard]] static TypeResult<std::string_view>
    get(lua_State *L, int index) {
        if (lua_type(L, index) != LUA_TSTRING)
            return makeErrorCode(ErrorCode::InvalidTypeCast);
 ...
 */
template <class T> struct Stack<std::optional<T>> {
 ...
 */
template <class T1, class T2> struct Stack<std::pair<T1, T2>> {
    ...
    [[nodiscard]] static TypeResult<std::pair<T1, T2>>
    get(lua_State *L, int index) {
  ...
  private:
    ...
    template <std::size_t Index>
    static Result
    pop_element(lua_State *L, int absIndex, std::pair<T1, T2> &p) {
        static_assert(Index < 2);
 ...
 */
template <class... Types> struct Stack<std::tuple<Types...>> {
    [[nodiscard]] static Result
    push(lua_State *L, const std::tuple<Types...> &t) {
#if LUABRIDGE_SAFE_STACK_CHECKS
        if (!lua_checkstack(L, 3))
            return makeErrorCode(ErrorCode::LuaStackOverflow);
#endif
    ...
    [[nodiscard]] static TypeResult<std::tuple<Types...>>
    get(lua_State *L, int index) {
        const StackRestore stackRestore(L);
    ...
    [[nodiscard]] static bool isInstance(lua_State *L, int index) {
        return lua_type(L, index) == LUA_TTABLE &&
               get_length(L, index) == static_cast<int>(Size);
    }
  ...
  private:
    ...
    template <std::size_t Index = 0>
        static auto
        pop_element(lua_State *L, int absIndex, std::tuple<Types...> &t)
            -> std::enable_if_t < Index<sizeof...(Types), Result> {
        using T = std::tuple_element_t<Index, std::tuple<Types...>>;
 ...
 */
template <class T, std::size_t N> struct Stack<T[N]> {
    ...
    [[nodiscard]] static bool isInstance(lua_State *L, int index) {
        return lua_type(L, index) == LUA_TTABLE &&
               get_length(L, index) == static_cast<int>(N);
    }
};
...
template <class T> struct StackOpSelector<const T &, false> {
    using ReturnType = TypeResult<T>;
...
template <class T> struct Stack<T &, std::enable_if_t<!std::is_array_v<T &>>> {
    ...
    [[nodiscard]] static bool isInstance(lua_State *L, int index) {
        return Helper::template isInstance<T>(L, index);
    }
};
...
struct Stack<const T &, std::enable_if_t<!std::is_array_v<const T &>>> {
    using Helper =
        detail::StackOpSelector<const T &, detail::IsUserdata<T>::value>;
    using ReturnType = typename Helper::ReturnType;
...
template <class T> struct Stack<const T *> {
    using Helper =
        detail::StackOpSelector<const T *, detail::IsUserdata<T>::value>;
    using ReturnType = typename Helper::ReturnType;
 ...
 */
template <class T> [[nodiscard]] Result push(lua_State *L, const T &t) {
 ...
 */
template <class T> [[nodiscard]] TypeResult<T> get(lua_State *L, int index) {
    return Stack<T>::get(L, index);
}
 ...
 */
template <class T> [[nodiscard]] bool isInstance(lua_State *L, int index) {
    return Stack<T>::isInstance(L, index);
}
