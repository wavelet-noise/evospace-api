 ...
 *
 * Construct one of these using `LuaNil ()` to represent a Lua nil. This is
 * faster than creating a reference in the registry to nil. Example:
 *
 * @code
 ...
 */
template <class Impl, class LuaRef> class LuaRefBase {
  protected:
  ...
  public:
     ...
     *
     * @returns An expected holding a value of the type T converted from this
     * reference or an error code.
     */
    template <class T> TypeResult<T> cast() const {
        const StackRestore stackRestore(m_L);
     ...
     */
    template <class T> T unsafe_cast() const {
        const StackRestore stackRestore(m_L);
     ...
     *
     * @returns True if the referred value is convertible to the type T, false
     * otherwise.
     */
    template <class T> bool isInstance() const {
        const StackRestore stackRestore(m_L);
     ...
     *
     * This operator calls cast<T> and always dereference the returned expected
     * instance, resulting in exceptions being thrown if the exceptions are
     * enabled, or otherwise we'll enter the UB land (and a likely crash down
     * the line).
     *
     * @returns A value of the type T converted from this reference.
     */
    template <class T> operator T() const { return cast<T>().value(); }
     ...
     * @param rhs A value to compare with.
     *
     * @returns True if the referred value is less than or equal to the
     * specified one.
     */
    template <class T> bool operator<=(const T &rhs) const {
        const StackRestore stackRestore(m_L);
     ...
     * @param rhs A value to compare with.
     *
     * @returns True if the referred value is greater than or equal to the
     * specified one.
     */
    template <class T> bool operator>=(const T &rhs) const {
        const StackRestore stackRestore(m_L);
     ...
     * @returns True if the referred value is equal to the specified one.
     */
    template <class T> bool rawequal(const T &v) const {
        const StackRestore stackRestore(m_L);
     ...
     *
     * If an error occurs, a LuaException is thrown (only if exceptions are
     * enabled).
     *
     * @returns A result of the call.
     */
    template <class... Args> LuaResult operator()(Args &&...args) const;
...
class LuaRef : public LuaRefBase<LuaRef, LuaRef> {
    ...
    class TableItem : public LuaRefBase<TableItem, LuaRef> {
      ...
      public:
        ...
        TableItem(lua_State *L, int tableRef)
            : LuaRefBase(L), m_keyRef(luaL_ref(L, LUA_REGISTRYINDEX)) {
#if LUABRIDGE_SAFE_STACK_CHECKS
         ...
         */
        TableItem(const TableItem &other) : LuaRefBase(other.m_L) {
#if LUABRIDGE_SAFE_STACK_CHECKS
         ...
         */
        template <class T> TableItem &operator=(const T &v) {
#if LUABRIDGE_SAFE_STACK_CHECKS
            if (!lua_checkstack(m_L, 2))
                return *this;
#endif
         ...
         * @returns This reference.
         */
        template <class T> TableItem &rawset(const T &v) {
#if LUABRIDGE_SAFE_STACK_CHECKS
            if (!lua_checkstack(m_L, 2))
                return *this;
#endif
         ...
         */
        template <class T> TableItem operator[](const T &key) const {
         ...
         */
        template <class T> LuaRef rawget(const T &key) const {
    ...
    //=========================================================================================
    /**
     * @brief Create a reference to an object at the top of the Lua stack and
     * pop it.
     *
     * This constructor is private and not invoked directly. Instead, use the
     * `fromStack` function.
     *
     * @param L A Lua state.
     *
  ...
  public:
     ...
     */
    template <class T>
    LuaRef(lua_State *L, const T &v) : LuaRefBase(L), m_ref(LUA_NOREF) {
        if (!Stack<T>::push(m_L, v))
            return;
     ...
     */
    LuaRef(const TableItem &v) : LuaRefBase(v.state()), m_ref(v.createRef()) {}
     ...
     */
    LuaRef(LuaRef &&other)
        : LuaRefBase(other.m_L), m_ref(std::exchange(other.m_ref, LUA_NOREF)) {}
     ...
     * caller to ensure that the thread still exists when the LuaRef is
     * destroyed.
     */
    ~LuaRef() {
        if (m_ref != LUA_NOREF)
    ...
    //=============================================================================================
    /**
     * @brief Create a new empty table on the top of a Lua stack and return a
     * reference to it.
     *
     * @param L A Lua state.
     *
     ...
     * @returns This reference.
     */
    template <class T> LuaRef &operator=(const T &rhs) {
        LuaRef ref(m_L, rhs);
        swap(ref);
        return *this;
    }
     ...
     * @brief Access a table value using a key.
     *
     * The operation is raw, metamethods are not invoked. The result is passed
     * by value and may not be modified.
     *
     * @param key A key in the table.
     *
     * @returns A reference to the table item.
 ...
 */
template <class T>
auto operator==(const T &lhs, const LuaRef &rhs) -> std::enable_if_t<
    !std::is_same_v<T, LuaRef> &&
        !std::is_same_v<T, LuaRefBase<LuaRef, LuaRef>>,
    bool> {
    return rhs == lhs;
}
 ...
 */
template <> struct Stack<LuaRef::TableItem> {
 ...
 */
template <class T> [[nodiscard]] TypeResult<T> cast(const LuaRef &ref) {
    return ref.cast<T>();
}
 ...
 */
template <class T> [[nodiscard]] T unsafe_cast(const LuaRef &ref) {
    return ref.unsafe_cast<T>();
}
} // namespace luabridge

namespace std {
template <> struct hash<luabridge::LuaRef> {
