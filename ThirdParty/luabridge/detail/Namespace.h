...
class Registrar {
  protected:
    ...
    Registrar(const Registrar &rhs)
        : L(rhs.L), m_stackSize(std::exchange(rhs.m_stackSize, 0)),
...
class Namespace : public detail::Registrar {
    ...
    class ClassBase : public detail::Registrar {
      ...
      protected:
        ...
        void createConstTable(const char *name, bool trueConst = true) {
            ...
            lua_rawsetp(
                L, -2, detail::getTypeKey()
            ); // co [typeKey] = name. Stack: ns, co
        ...
        void createClassTable(const char *name) {
            ...
            lua_rawsetp(
                L, -2, detail::getPropsetKey()
            ); // cl [propsetKey] = ps. Stack: ns, co, cl

            lua_pushvalue(L, -2); // Stack: ns, co, cl, co
            lua_rawsetp(
                L, -2, detail::getConstKey()
            ); // cl [constKey] = co. Stack: ns, co, cl

            lua_pushvalue(L, -1); // Stack: ns, co, cl, cl
            lua_rawsetp(
                L, -3, detail::getClassKey()
            ); // co [classKey] = cl. Stack: ns, co, cl
        }
        ...
        void createStaticTable(const char *name) {
            ...
            lua_setmetatable(
                L, -3
            ); // st.__metatable = mt. Stack: ns, co, cl, vst, st
            ...
            lua_rawsetp(
                L, -2, detail::getPropgetKey()
            ); // st [propgetKey] = pg. Stack: ns, co, cl, st

            lua_newtable(L); // Stack: ns, co, cl, st, propset table (ps)
            lua_rawsetp(
                L, -2, detail::getPropsetKey()
            ); // st [propsetKey] = pg. Stack: ns, co, cl, st

            lua_pushvalue(L, -2); // Stack: ns, co, cl, st, cl
            lua_rawsetp(
                L, -2, detail::getClassKey()
            ); // st [classKey] = cl. Stack: ns, co, cl, st
     ...
     */
    template <class T> class Class : public ClassBase {
      public:
        //=========================================================================================
         ...
         */
        Class(const char *name, Namespace &parent) : ClassBase(parent) {
            assert(name != nullptr);
            ...
            if (lua_isnil(L, -1)) // Stack: ns, nil
            {
                ...
                rawsetfield(
                    L, -2, "__gc"
                ); // cl ["__gc"] = function. Stack: ns, co, cl
#endif
                ++m_stackSize;
                ...
                lua_rawsetp(
                    L, LUA_REGISTRYINDEX, detail::getStaticRegistryKey<T>()
                );                    // Stack: ns, co, cl, st
                lua_pushvalue(L, -2); // Stack: ns, co, cl, st, cl
                lua_rawsetp(
                    L, LUA_REGISTRYINDEX, detail::getClassRegistryKey<T>()
                );                    // Stack: ns, co, cl, st
                lua_pushvalue(L, -3); // Stack: ns, co, cl, st, co
                lua_rawsetp(
                    L, LUA_REGISTRYINDEX, detail::getConstRegistryKey<T>()
                ); // Stack: ns, co, cl, st
            } else {
        ...
        Class(const char *name, Namespace &parent, void const *const staticKey)
            ...
            lua_rawgetp(
                L, LUA_REGISTRYINDEX, staticKey
            ); // Stack: ns, co, cl, st, parent st (pst) | nil
            ...
            lua_rawgetp(
                L, -1, detail::getClassKey()
            ); // Stack: ns, co, cl, st, pst, parent cl (pcl)
            assert(lua_istable(L, -1));

            lua_rawgetp(
                L, -1, detail::getConstKey()
            ); // Stack: ns, co, cl, st, pst, pcl, parent co (pco)
            assert(lua_istable(L, -1));

            lua_rawsetp(
                L, -6, detail::getParentKey()
            ); // co [parentKey] = pco. Stack: ns, co, cl, st, pst, pcl
            lua_rawsetp(
                L, -4, detail::getParentKey()
            ); // cl [parentKey] = pcl. Stack: ns, co, cl, st, pst
            lua_rawsetp(
                L, -2, detail::getParentKey()
            ); // st [parentKey] = pst. Stack: ns, co, cl, st
        ...
        Class<T> &addStaticProperty(const char *name, const U *value) {
            ...
            lua_pushlightuserdata(
                L, const_cast<U *>(value)
            ); // Stack: co, cl, st, pointer
            lua_pushcclosure_x(
                L, &detail::property_getter<U>::call, 1
            ); // Stack: co, cl, st, getter
            detail::add_property_getter(L, name, -2); // Stack: co, cl, st

            lua_pushstring(L, name); // Stack: co, cl, st, name
            lua_pushcclosure_x(
                L, &detail::read_only_error, 1
            ); // Stack: co, cl, st, function
        ...
        addStaticProperty(const char *name, U *value, bool isWritable = true) {
            ...
            if (isWritable) {
                lua_pushlightuserdata(
                    L, value
                ); // Stack: co, cl, st, ps, pointer
                lua_pushcclosure_x(
                    L, &detail::property_setter<U>::call, 1
                ); // Stack: co, cl, st, ps, setter
            } else {
                lua_pushstring(L, name); // Stack: co, cl, st, name
                lua_pushcclosure_x(
                    L, &detail::read_only_error, 1
        ...
        Class<T> &addStaticProperty(
            ...
            lua_pushlightuserdata(
                L, reinterpret_cast<void *>(get)
            ); // Stack: co, cl, st, function ptr
            lua_pushcclosure_x(
                L, &detail::invoke_proxy_function<U (*)()>, 1
            ); // Stack: co, cl, st, getter
            detail::add_property_getter(L, name, -2); // Stack: co, cl, st

            if (set != nullptr) {
                lua_pushlightuserdata(
                    L, reinterpret_cast<void *>(set)
                ); // Stack: co, cl, st, function ptr
                lua_pushcclosure_x(
                    L, &detail::invoke_proxy_function<void (*)(U)>, 1
                ); // Stack: co, cl, st, setter
            } else {
        ...
        Class<T> &addStaticProperty(
            ...
            lua_pushcclosure_x(
                L, &detail::invoke_proxy_function<U (*)() noexcept>, 1
            ); // Stack: co, cl, st, getter
            detail::add_property_getter(L, name, -2); // Stack: co, cl, st
            ...
            if (set != nullptr) {
                ...
                lua_pushcclosure_x(
                    L, &detail::invoke_proxy_function<void (*)(U) noexcept>, 1
                ); // Stack: co, cl, st, setter
            } else {
                lua_pushstring(L, name); // Stack: co, cl, st, ps, name
                lua_pushcclosure_x(
        ...
        Class<T> &addStaticProperty(const char *name, Getter get) {
            ...
            lua_newuserdata_aligned<GetType>(
                L, std::move(get)
            ); // Stack: co, cl, st, function userdata (ud)
            lua_pushcclosure_x(
                L, &detail::invoke_proxy_functor<GetType>, 1
            ); // Stack: co, cl, st, function
            detail::add_property_getter(L, name, -2); // Stack: co, cl, st
        ...
        Class<T> &addStaticProperty(const char *name, Getter get, Setter set) {
            ...
            lua_pushcclosure_x(
                L, &detail::invoke_proxy_functor<SetType>, 1
            ); // Stack: co, cl, st, function
            detail::add_property_setter(L, name, -2); // Stack: co, cl, st
        ...
        auto addStaticFunction(const char *name, Functions... functions)
            ...
            if constexpr (sizeof...(Functions) == 1) {
                ...
                lua_createtable(
                    L, static_cast<int>(sizeof...(Functions)), 0
                ); // reserve space for N overloads
                ...
                (
                    [&] {
                        lua_createtable(
                            L, 2, 0
                        ); // reserve space for: function, arity
                        lua_pushinteger(L, 1);
        ...
        addProperty(const char *name, U V::*mp, bool isWritable = true) {
            ...
            lua_pushcclosure_x(
                L, &detail::property_getter<U, T>::call, 1
            );                    // Stack: co, cl, st, getter
            ...
            if (isWritable) {
                new (lua_newuserdata_x<MemberPtrType>(L, sizeof(MemberPtrType)))
                    MemberPtrType(mp); // Stack: co, cl, st, field ptr
                lua_pushcclosure_x(
                    L, &detail::property_setter<U, T>::call, 1
                ); // Stack: co, cl, st, setter
        ...
        Class<T> &addProperty(
            ...
            lua_pushcclosure_x(
                L, &detail::invoke_const_member_function<GetType, T>, 1
            );                    // Stack: co, cl, st, getter
            lua_pushvalue(L, -1); // Stack: co, cl, st, getter, getter
            detail::add_property_getter(
                L, name, -5
            ...
            if (set != nullptr) {
                ...
                lua_pushcclosure_x(
                    L, &detail::invoke_member_function<SetType, T>, 1
                ); // Stack: co, cl, st, setter
                detail::add_property_setter(L, name, -3); // Stack: co, cl, st
            }
        ...
        Class<T> &addProperty(
            ...
            lua_pushcclosure_x(
                L, &detail::invoke_proxy_function<TG (*)(const T *)>, 1
            );                    // Stack: co, cl, st, getter
            lua_pushvalue(L, -1); // Stack: co, cl, st,, getter, getter
            detail::add_property_getter(
                L, name, -5
            ...
            if (set != nullptr) {
                ...
                lua_pushcclosure_x(
                    L, &detail::invoke_proxy_function<void (*)(T *, TS)>, 1
                ); // Stack: co, cl, st, setter
                detail::add_property_setter(L, name, -3); // Stack: co, cl, st
            }
        ...
        Class<T> &addProperty(
            ...
            lua_pushcclosure_x(
                L, &detail::invoke_proxy_function<TG (*)(const T *) noexcept>, 1
            );                    // Stack: co, cl, st, getter
            lua_pushvalue(L, -1); // Stack: co, cl, st,, getter, getter
            detail::add_property_getter(
                L, name, -5
        ...
        auto addFunction(const char *name, Functions... functions)
            ...
            if constexpr (sizeof...(Functions) == 1) {
                ...
                if constexpr (detail::const_functions_count<T, Functions...> == 1) {
                    lua_pushvalue(
                        L, -1
                    ); // Stack: co, cl, st, function, function
        ...
        auto addIndexMetaMethod(Function function) -> std::enable_if_t<
            ...
            lua_pushcclosure_x(
                L, &detail::invoke_proxy_functor<FnType>, 1
            ); // Stack: co, cl, st, function
        ...
        Class<T> &addIndexMetaMethod(LuaRef (*idxf
            ...
            lua_pushlightuserdata(
                L, reinterpret_cast<void *>(idxf)
            ); // Stack: co, cl, st, function ptr
            lua_pushcclosure_x(
                L, &detail::invoke_proxy_function<FnType>, 1
            ); // Stack: co, cl, st, function
            lua_rawsetp(L, -3, detail::getIndexFallbackKey());
  ...
  private:
     ...
     * @param L A Lua state.
     */
    Namespace(lua_State *L, FromStack) : Registrar(L, 1) {
        assert(lua_istable(L, -1));

        {
            ...
            lua_rawsetp(
                L, -2, detail::getPropgetKey()
            ); // ns [propgetKey] = pg. Stack: ns

            lua_newtable(L); // Stack: ns, mt, propset table (ps)
            lua_rawsetp(
                L, -2, detail::getPropsetKey()
            ); // ns [propsetKey] = ps. Stack: ns
        }
     ...
     */
    Namespace(const char *name, Namespace &parent) : Registrar(parent) {
        assert(name != nullptr);
        ...
        if (lua_isnil(L, -1)) // Stack: pns, nil
        {
            ...
            lua_rawsetp(
                L, -2, detail::getPropgetKey()
            ); // ns [propgetKey] = pg. Stack: pns, ns

            lua_newtable(L); // Stack: pns, ns, propset table (ps)
            lua_rawsetp(
                L, -2, detail::getPropsetKey()
            ); // ns [propsetKey] = ps. Stack: pns, ns
    ...
    explicit Namespace(Table &child) : Registrar(child) {}
  ...
  public:
    ...
    Namespace &addProperty(const char *name, T *value, bool isWritable = true) {
        ...
        lua_pushcclosure_x(
            L, &detail::property_getter<T>::call, 1
        );                                        // Stack: ns, getter
        ...
        if (isWritable) {
            lua_pushlightuserdata(L, value); // Stack: ns, pointer
            lua_pushcclosure_x(
                L, &detail::property_setter<T>::call, 1
            ); // Stack: ns, setter
        } else {
    ...
    addProperty(const char *name, TG (*get)(), void (*set)(TS) = nullptr) {
        ...
        lua_pushlightuserdata(
            L, reinterpret_cast<void *>(get)
        ); // Stack: ns, function ptr
        lua_pushcclosure_x(
            L, &detail::invoke_proxy_function<TG (*)()>, 1
        ); // Stack: ns, getter
    ...
    Namespace &addProperty(
        ...
        lua_pushcclosure_x(
            L, &detail::invoke_proxy_function<TG (*)() noexcept>, 1
        ); // Stack: ns, getter
        detail::add_property_getter(L, name, -2);
     ...
     */
    template <class T> Class<T> beginClass(const char *name) {
        assertIsActive();
 ...
 * @returns A namespace registration object.
 */
inline void registerMainThread(lua_State *L) { register_main_thread(L); }
