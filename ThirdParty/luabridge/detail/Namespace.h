 ...
 *
 * Maintains Lua stack in the proper state. Once beginNamespace, beginClass or
 * deriveClass is called the parent object upon its destruction may no longer
 * clear the Lua stack.
 *
 * Then endNamespace or endClass is called, a new parent is created and the
 * child transfers the responsibility for clearing stack to it.
 *
...
protected:
    ...
    Registrar(const Registrar &rhs)
      : L(rhs.L), m_stackSize(std::exchange(rhs.m_stackSize, 0)), m_skipStackPops(std::exchange(rhs.m_skipStackPops, 0)) {}
...
protected:
    ...
    void createConstTable(const char *name, bool trueConst = true) {
      ...
      std::string type_name =
        std::string(trueConst ? "const " : "") + name;
      ...
      lua_rawsetp(
        L, -2, detail::getTypeKey()); // co [typeKey] = name. Stack: ns, co
    ...
    void createClassTable(const char *name) {
      ...
      lua_rawsetp(
        L, -2, detail::getPropsetKey()); // cl [propsetKey] = ps. Stack: ns, co, cl

      lua_pushvalue(L, -2); // Stack: ns, co, cl, co
      lua_rawsetp(
        L, -2, detail::getConstKey()); // cl [constKey] = co. Stack: ns, co, cl

      lua_pushvalue(L, -1); // Stack: ns, co, cl, cl
      lua_rawsetp(
        L, -3, detail::getClassKey()); // co [classKey] = cl. Stack: ns, co, cl
    }
    ...
    void createStaticTable(const char *name) {
      ...
      lua_setmetatable(
        L, -3); // st.__metatable = mt. Stack: ns, co, cl, vst, st
      ...
      lua_rawsetp(
        L, -2, detail::getClassKey()); // st [classKey] = cl. Stack: ns, co, cl, st
...
public:
    ...
    Class(const char *name, Namespace &parent)
      ...
      if (lua_isnil(L, -1)) // Stack: ns, nil
      {
        ...
        lua_pushcfunction_x(L, &detail::gc_metamethod<T>); // Stack: ns,
          // co,
          // function
        rawsetfield(
          L, -2, "__gc"); // co ["__gc"] = function. Stack: ns, co
#endif
        ++m_stackSize;
        ...
        lua_pushcfunction_x(L, &detail::gc_metamethod<T>); // Stack: ns,
          ...
          // function
        rawsetfield(
          L, -2, "__gc"); // cl ["__gc"] = function. Stack: ns, co, cl
#endif
        ++m_stackSize;
        ...
        lua_rawgetp(
          L, LUA_REGISTRYINDEX, detail::getClassRegistryKey<T>()); // Stack: ns, co, st, cl
    ...
    Class(const char *name, Namespace &parent, void const *const staticKey)
      ...
      createConstTable(name); // Stack: ns, const table (co)
#if !defined(LUABRIDGE_ON_LUAU)
      lua_pushcfunction_x(L, &detail::gc_metamethod<T>); // Stack: ns, co,
        // function
      ...
      lua_rawsetp(
        L, -6, detail::getParentKey()); // co [parentKey] = pco. Stack: ns, co, cl, st, pst, pcl
      lua_rawsetp(
    ...
    /**
         ...
         * @param isWritable True for a read-write, false for read-only
         * property.
         *
    ...
    Class<T> &addStaticProperty(const char *name, const U *value) {
      ...
      lua_pushlightuserdata(
        L, const_cast<U *>(value)); // Stack: co, cl, st, pointer
      lua_pushcclosure_x(
        L, &detail::property_getter<U>::call, 1); // Stack: co, cl, st, getter
      detail::add_property_getter(L, name, -2); // Stack: co, cl, st

      lua_pushstring(L, name); // Stack: co, cl, st, name
      lua_pushcclosure_x(
        L, &detail::read_only_error, 1); // Stack: co, cl, st, function
    ...
    template <class U, class = std::enable_if_t<!std::is_invocable_v<U>>>
    Class<T> &
    addStaticProperty(const char *name, U *value, bool isWritable = true) {
      assert(name != nullptr);
      assertStackState(); // Stack: const table (co), class table (cl), static table (st)
      ...
      if (isWritable) {
        lua_pushlightuserdata(
          L, value); // Stack: co, cl, st, ps, pointer
        lua_pushcclosure_x(
          L, &detail::property_setter<U>::call, 1); // Stack: co, cl, st, ps, setter
      } else {
        lua_pushstring(L, name); // Stack: co, cl, st, name
        lua_pushcclosure_x(
          L, &detail::read_only_error, 1); // Stack: co, cl, st, function
    ...
    /**
         ...
         */
    template <class U>
    Class<T> &addStaticProperty(
      const char *name, U (*get)(), void (*set)(U) = nullptr) {
      assert(name != nullptr);
      assertStackState(); // Stack: const table (co), class table (cl), static table (st)

      lua_pushlightuserdata(
        L, reinterpret_cast<void *>(get)); // Stack: co, cl, st, function ptr
      lua_pushcclosure_x(
        L, &detail::invoke_proxy_function<U (*)()>, 1); // Stack: co, cl, st, getter
      detail::add_property_getter(L, name, -2); // Stack: co, cl, st
      ...
      if (set != nullptr) {
        ...
        lua_pushcclosure_x(
          L, &detail::invoke_proxy_function<void (*)(U)>, 1); // Stack: co, cl, st, setter
      } else {
    ...
    template <class U>
    Class<T> &addStaticProperty(
      const char *name, U (*get)() noexcept,
      void (*set)(U) noexcept = nullptr) {
      assert(name != nullptr);
      assertStackState(); // Stack: const table (co), class table (cl), static table (st)
      ...
      lua_pushlightuserdata(
        L, reinterpret_cast<void *>(get)); // Stack: co, cl, st, function ptr
      lua_pushcclosure_x(
        L, &detail::invoke_proxy_function<U (*)() noexcept>, 1); // Stack: co, cl, st, getter
      detail::add_property_getter(L, name, -2); // Stack: co, cl, st
      ...
      if (set != nullptr) {
        lua_pushlightuserdata(
          L, reinterpret_cast<void *>(set)); // Stack: co, cl, st, function ptr
        lua_pushcclosure_x(
          L, &detail::invoke_proxy_function<void (*)(U) noexcept>, 1); // Stack: co, cl, st, setter
      } else {
        lua_pushstring(L, name); // Stack: co, cl, st, ps, name
        lua_pushcclosure_x(
          L, &detail::read_only_error, 1); // Stack: co, cl, st, function
    ...
    //=========================================================================================
    /**
         * @brief Add or replace a static property, by constructible by
         * std::function.
         */
    template <
      class Getter, class = std::enable_if_t<!std::is_pointer_v<Getter>>>
    Class<T> &addStaticProperty(const char *name, Getter get) {
      ...
      lua_newuserdata_aligned<GetType>(
        L, std::move(get)); // Stack: co, cl, st, function userdata (ud)
      lua_pushcclosure_x(
        L, &detail::invoke_proxy_functor<GetType>, 1); // Stack: co, cl, st, function
      detail::add_property_getter(L, name, -2); // Stack: co, cl, st
    ...
    template <
      ...
      class = std::enable_if_t<
        !std::is_pointer_v<Getter> && !std::is_pointer_v<Setter>>>
    Class<T> &addStaticProperty(const char *name, Getter get, Setter set) {
      ...
      lua_newuserdata_aligned<SetType>(
        L, std::move(set)); // Stack: co, cl, st, function userdata (ud)
      lua_pushcclosure_x(
        L, &detail::invoke_proxy_functor<SetType>, 1); // Stack: co, cl, st, function
      detail::add_property_setter(L, name, -2); // Stack: co, cl, st
    ...
    //=========================================================================================
    /**
         * @brief Add or replace a single static function or multiple overloaded
         * functions.
         *
         * @param name The overload name.
         * @param functions A single or set of static functions that will be
         * invoked.
         *
         * @returns This class registration object.
         */
    ...
    auto addStaticFunction(const char *name, Functions... functions)
      -> std::enable_if_t<
        (detail::is_callable_v<Functions> && ...) &&
          (sizeof...(Functions) > 0),
        Class<T> &> {
      assert(name != nullptr);
      assertStackState(); // Stack: const table (co), class table (cl), static table (st)
      ...
      if constexpr (sizeof...(Functions) == 1) {
        ...
        lua_createtable(
          L, static_cast<int>(sizeof...(Functions)), 0); // reserve space for N overloads
        ...
        (
          [&] {
            lua_createtable(
              L, 2, 0); // reserve space for: function, arity
            lua_pushinteger(L, 1);
            if constexpr (detail::is_any_cfunction_pointer_v<
                            Functions>)
              lua_pushinteger(L, -1);
            else
              lua_pushinteger(
                L,
                static_cast<
                  int>(detail::function_arity_excluding_v<
                       Functions,
                       lua_State *>));
            lua_settable(L, -3);
            lua_pushinteger(L, 2);
        ...
        lua_pushcclosure_x(
          L, &detail::try_overload_functions<false>, 1);
      }
    ...
    Class<T> &
    addProperty(const char *name, U V::*mp, bool isWritable = true) {
      ...
      new (lua_newuserdata_x<MemberPtrType>(L, sizeof(MemberPtrType)))
        MemberPtrType(mp); // Stack: co, cl, st, field ptr
      lua_pushcclosure_x(
        L, &detail::property_getter<U, T>::call, 1); // Stack: co, cl, st, getter
      lua_pushvalue(L, -1); // Stack: co, cl, st, getter, getter
      detail::add_property_getter(
        L, name, -5); // Stack: co, cl, st, getter
      ...
      if (isWritable) {
        new (lua_newuserdata_x<MemberPtrType>(L, sizeof(MemberPtrType)))
          MemberPtrType(mp); // Stack: co, cl, st, field ptr
        lua_pushcclosure_x(
          L, &detail::property_setter<U, T>::call, 1); // Stack: co, cl, st, setter
    ...
    Class<T> &addProperty(
      const char *name, TG (T::*get)() const, void (T::*set)(TS) = nullptr) {
      ...
      new (lua_newuserdata_x<GetType>(L, sizeof(GetType)))
        GetType(get); // Stack: co, cl, st, funcion ptr
      lua_pushcclosure_x(
        L, &detail::invoke_const_member_function<GetType, T>, 1); // Stack: co, cl, st, getter
      lua_pushvalue(L, -1); // Stack: co, cl, st, getter, getter
      detail::add_property_getter(
        L, name, -5); // Stack: co, cl, st, getter
      detail::add_property_getter(L, name, -3); // Stack: co, cl, st
      ...
      if (set != nullptr) {
        new (lua_newuserdata_x<SetType>(L, sizeof(SetType)))
          SetType(set); // Stack: co, cl, st, function ptr
        lua_pushcclosure_x(
          L, &detail::invoke_member_function<SetType, T>, 1); // Stack: co, cl, st, setter
        detail::add_property_setter(L, name, -3); // Stack: co, cl, st
      }
    ...
    template <class TG, class TS = TG>
    Class<T> &addProperty(
      const char *name, TG (T::*get)() const noexcept,
      void (T::*set)(TS) noexcept = nullptr) {
    ...
    template <class TG, class TS = TG>
    Class<T> &addProperty(
      const char *name, TG (T::*get)(lua_State *) const,
      void (T::*set)(TS, lua_State *) = nullptr) {
    ...
    template <class TG, class TS = TG>
    Class<T> &addProperty(
      const char *name, TG (T::*get)(lua_State *) const noexcept,
      void (T::*set)(TS, lua_State *) noexcept = nullptr) {
    ...
    /**
         ...
         *
         * When a class is closed for modification and does not provide (or
         * cannot provide) the function signatures necessary to implement get or
         * set for a property, this will allow non-member functions act as
         * proxies.
         *
         * Both the get and the set functions require a T const* and T* in the
         * first argument respectively.
         */
    template <class TG, class TS = TG>
    Class<T> &addProperty(
      const char *name, TG (*get)(const T *),
      void (*set)(T *, TS) = nullptr) {
      assert(name != nullptr);
      assertStackState(); // Stack: const table (co), class table (cl), static table (st)
      ...
      lua_pushlightuserdata(
        L, reinterpret_cast<void *>(get)); // Stack: co, cl, st, function ptr
      lua_pushcclosure_x(
        L, &detail::invoke_proxy_function<TG (*)(const T *)>, 1); // Stack: co, cl, st, getter
      lua_pushvalue(L, -1); // Stack: co, cl, st,, getter, getter
      detail::add_property_getter(
        L, name, -5); // Stack: co, cl, st, getter
      detail::add_property_getter(L, name, -3); // Stack: co, cl, st
      ...
      if (set != nullptr) {
        lua_pushlightuserdata(
          L, reinterpret_cast<void *>(set)); // Stack: co, cl, st, function ptr
        lua_pushcclosure_x(
          L, &detail::invoke_proxy_function<void (*)(T *, TS)>, 1); // Stack: co, cl, st, setter
        detail::add_property_setter(L, name, -3); // Stack: co, cl, st
      }
    ...
    template <class TG, class TS = TG>
    Class<T> &addProperty(
      const char *name, TG (*get)(const T *) noexcept,
      void (*set)(T *, TS) noexcept = nullptr) {
      assert(name != nullptr);
      assertStackState(); // Stack: const table (co), class table (cl), static table (st)
      ...
      lua_pushlightuserdata(
        L, reinterpret_cast<void *>(get)); // Stack: co, cl, st, function ptr
      lua_pushcclosure_x(
        L, &detail::invoke_proxy_function<TG (*)(const T *) noexcept>, 1); // Stack: co, cl, st, getter
      lua_pushvalue(L, -1); // Stack: co, cl, st,, getter, getter
      detail::add_property_getter(
        L, name, -5); // Stack: co, cl, st, getter
      detail::add_property_getter(L, name, -3); // Stack: co, cl, st
      ...
      if (set != nullptr) {
        ...
        lua_pushcclosure_x(
          L,
          &detail::invoke_proxy_function<void (*)(T *, TS) noexcept>,
          1); // Stack: co, cl, st, setter
        detail::add_property_setter(L, name, -3); // Stack: co, cl, st
      }
    ...
    /**
         ...
         */
    Class<T> &addProperty(
      const char *name, lua_CFunction get, lua_CFunction set = nullptr) {
      assert(name != nullptr);
      assertStackState(); // Stack: const table (co), class table (cl), static table (st)
    ...
    //=========================================================================================
    /**
         * @brief Add or replace a property member, by constructible by
         * std::function.
         */
    template <
      class Getter, class = std::enable_if_t<!std::is_pointer_v<Getter>>>
    Class<T> &addProperty(const char *name, Getter get) {
      ...
      static_assert(std::is_same_v<
                    std::decay_t<std::remove_pointer_t<FirstArg>>,
                    T>);
      ...
      lua_newuserdata_aligned<GetType>(
        L, std::move(get)); // Stack: co, cl, st, function userdata (ud)
      lua_pushcclosure_x(
        L, &detail::invoke_proxy_functor<GetType>, 1); // Stack: co, cl, st, getter
      lua_pushvalue(L, -1); // Stack: co, cl, st, getter, getter
      detail::add_property_getter(
        L, name, -4); // Stack: co, cl, st, getter
    ...
    Class<T> &addProperty(const char *name, Getter get, Setter set) {
      ...
      lua_newuserdata_aligned<SetType>(
        L, std::move(set)); // Stack: co, cl, st, function userdata (ud)
      lua_pushcclosure_x(
        L, &detail::invoke_proxy_functor<SetType>, 1); // Stack: co, cl, st, setter
      detail::add_property_setter(L, name, -3); // Stack: co, cl, st
    ...
    auto addFunction(const char *name, Functions... functions)
      ...
      if (name == std::string_view("__gc")) {
        throw_or_assert<std::logic_error>(
          "__gc metamethod registration is forbidden");
        return *this;
      }
      ...
      if constexpr (sizeof...(Functions) == 1) {
        (
          [&] {
            detail::push_member_function<T>(
              L, std::move(functions));
          }(),
          ...);

        if constexpr (detail::const_functions_count<T, Functions...> == 1) {
          lua_pushvalue(
            L, -1); // Stack: co, cl, st, function, function
        ...
        // create new closure of const try_overload_functions with new
        // table
        if constexpr (detail::const_functions_count < T, Functions... >> 0) {
          lua_createtable(
            L,
            static_cast<int>(detail::const_functions_count<
                             T,
                             Functions...>),
            0);
            ...
            [&] {
              ...
              if constexpr (detail::is_any_cfunction_pointer_v<
                ...
                lua_pushinteger(
                  ...
                  static_cast<
                    int>(detail::
                           member_function_arity_excluding_v<
                             T,
                             Functions,
                             lua_State *>));
              lua_settable(L, -3);
          ...
          lua_pushcclosure_x(
            L, &detail::try_overload_functions<true>, 1);
          lua_pushvalue(
            L, -1); // Stack: co, cl, st, function, function
          rawsetfield(L, -4, name); // Stack: co, cl, st, function
          rawsetfield(L, -4, name); // Stack: co, cl, st
        ...
        // create new closure of non const try_overload_functions with
        // new table
        if constexpr (detail::non_const_functions_count < T, Functions... >> 0) {
          lua_createtable(
            L,
            static_cast<int>(detail::non_const_functions_count<
                             T,
                             Functions...>),
            0);
    ...
    /**
         ...
         * the desired Constructor (since you can't take the address of a
         * Constructor and pass it as an argument).
         */
    template <class... Functions>
    auto addConstructor()
      ...
      if constexpr (sizeof...(Functions) == 1) {
        (
          [&] {
            lua_pushcclosure_x(
              L,
              &detail::constructor_placement_proxy<
                T,
                detail::function_arguments_t<Functions>>,
              0);
          }(),
          ...);
      } else {
        ...
        (
          [&] {
            ...
            lua_pushinteger(
              L,
              static_cast<int>(detail::function_arity_excluding_v<
                               Functions,
                               lua_State *>));
            lua_settable(L, -3);
            lua_pushinteger(L, 2);
    ...
    //=========================================================================================
    /**
         * @brief Add or replace a primary Constructor when the type is used
         * from an intrusive container C.
         */
      ...
      if constexpr (sizeof...(Functions) == 1) {
        (
          [&] {
            lua_pushcclosure_x(
              L,
              &detail::constructor_container_proxy<
                C,
                detail::function_arguments_t<Functions>>,
              0);
          }(),
    ...
    /**
         ...
         *
         * The primary placement constructor is invoked when calling the class
         * type table like a function.
         *
         * The provider of the Function argument is responsible of doing
         * placement new of the type T over the void* pointer provided to the
         * method as first argument.
         */
    template <class... Functions>
    auto addConstructor(Functions... functions) -> std::enable_if_t<
      (detail::is_callable_v<Functions> && ...) &&
        (sizeof...(Functions) > 0),
      Class<T> &> {
      static_assert(
        ((detail::function_arity_excluding_v<Functions, lua_State *> >=
         ...
         ...));
      static_assert(((std::is_same_v<
                      detail::function_argument_t<0, Functions>,
                      void *>)&&...));
      ...
      if constexpr (sizeof...(Functions) == 1) {
        (
          [&] {
            detail::push_function(
              L,
              detail::constructor_forwarder<T, Functions>(
                std::move(functions))); // Stack: co, cl, st, function
          }(),
          ...);
      } else {
        // create new closure of try_overloads with new table
    ...
    Class<T> &addFactory(Allocator allocator, Deallocator deallocator) {
      ...
      detail::push_function(
        L,
        detail::factory_forwarder<T, Allocator, Deallocator>(
          std::move(allocator), std::move(deallocator)));
      rawsetfield(L, -2, "__call"); // Stack: co, cl, st
    ...
    //=========================================================================================
    /**
         * @brief Add an index metamethod function fallback that is triggered
         * when no result is found in functions, properties or any other
         * members.
         *
         * Let the user define a fallback index (__index) metamethod at its
         * level.
         */
    template <class Function>
    auto addIndexMetaMethod(Function function) -> std::enable_if_t<
      !std::is_pointer_v<Function> &&
        std::is_invocable_v<Function, T &, const LuaRef &, lua_State *>,
      Class<T> &> {
      using FnType = decltype(function);

      assertStackState(); // Stack: const table (co), class table (cl), static table (st)

      lua_newuserdata_aligned<FnType>(
        L, std::move(function)); // Stack: co, cl, st, function userdata (ud)
      lua_pushcclosure_x(
        L, &detail::invoke_proxy_functor<FnType>, 1); // Stack: co, cl, st, function
    ...
    addIndexMetaMethod(LuaRef (*idxf)(T &, const LuaRef &, lua_State *)) {
      ...
      lua_pushlightuserdata(
        L, reinterpret_cast<void *>(idxf)); // Stack: co, cl, st, function ptr
      lua_pushcclosure_x(
        L, &detail::invoke_proxy_function<FnType>, 1); // Stack: co, cl, st, function
      lua_rawsetp(L, -3, detail::getIndexFallbackKey());
    ...
    addIndexMetaMethod(LuaRef (T::*idxf)(const LuaRef &, lua_State *)) {
      ...
      new (lua_newuserdata_x<MemFnPtr>(L, sizeof(MemFnPtr)))
        MemFnPtr(idxf);
      lua_pushcclosure_x(
        L, &detail::invoke_member_function<MemFnPtr, T>, 1);
      lua_rawsetp(L, -3, detail::getIndexFallbackKey());
    ...
    //=========================================================================================
    /**
         * @brief Add an insert index metamethod function fallback that is
         * triggered when no result is found in functions, properties or any
         * other members.
         *
         * Let the user define a fallback insert index (___newindex) metamethod
         * at its level.
         */
    template <class Function>
    auto addNewIndexMetaMethod(Function function) -> std::enable_if_t<
      !std::is_pointer_v<Function> &&
        std::is_invocable_v<
          Function, T &, const LuaRef &, const LuaRef &, lua_State *>,
      Class<T> &> {
      using FnType = decltype(function);
...
public:
    ...
    Table &addFunction(const char *name, Function function) {
      ...
      lua_newuserdata_aligned<FnType>(
        L, std::move(function)); // Stack: ns, function userdata (ud)
      lua_pushcclosure_x(
        L, &detail::invoke_proxy_functor<FnType>, 1); // Stack: ns, function
  ...
  //=============================================================================================
  /**
     * @brief Open the a namespace for registrations from a table on top of the
     * stack.
     *
     * @param L A Lua state.
     */
  Namespace(lua_State *L, FromStack)
...

    {
      ...
      lua_rawsetp(
        L, -2, detail::getPropgetKey()); // ns [propgetKey] = pg. Stack: ns

      lua_newtable(L); // Stack: ns, mt, propset table (ps)
      lua_rawsetp(
        L, -2, detail::getPropsetKey()); // ns [propsetKey] = ps. Stack: ns
    }
  ...
  Namespace(const char *name, Namespace &parent)
    ...
    if (lua_isnil(L, -1)) // Stack: pns, nil
    {
      ...
      lua_rawsetp(
        L, -2, detail::getPropgetKey()); // ns [propgetKey] = pg. Stack: pns, ns

      lua_newtable(L); // Stack: pns, ns, propset table (ps)
      lua_rawsetp(
        L, -2, detail::getPropsetKey()); // ns [propsetKey] = ps. Stack: pns, ns
  ...
  /**
     ...
     * then add your classes and functions to it, rather than adding many
     * classes and functions directly to the global namespace.
     *
     * @param L A Lua state.
     *
     * @returns A namespace registration object.
     */
  static Namespace getGlobalNamespace(lua_State *L) { return Namespace(L); }

  /**
     * @brief Retrieve the namespace on top of the stack.
     *
     * You should have a table on top of the stack before calling this function.
     * It will then use the table there as destination for registrations.
     *
     * @param L A Lua state.
     *
     * @returns A namespace registration object.
  ...
  Namespace endNamespace() {
    if (m_stackSize == 1) {
      throw_or_assert<std::logic_error>(
        "endNamespace() called on global namespace");
  ...
  //=============================================================================================
  /**
     * @brief Add or replace a variable, a variable will be added in the
     * namespace by copy of the passed value.
     *
     * @param name The property name.
  ...
  Namespace &addVariable(const char *name, const T &value) {
    if (m_stackSize == 1) {
      throw_or_assert<std::logic_error>(
        "addVariable() called on global namespace");
  ...
  Namespace &addProperty(const char *name, T *value, bool isWritable = true) {
    if (m_stackSize == 1) {
      throw_or_assert<std::logic_error>(
        "addProperty() called on global namespace");
    ...
    lua_pushcclosure_x(
      L, &detail::property_getter<T>::call, 1); // Stack: ns, getter
    detail::add_property_getter(L, name, -2); // Stack: ns

    if (isWritable) {
      lua_pushlightuserdata(L, value); // Stack: ns, pointer
      lua_pushcclosure_x(
        L, &detail::property_setter<T>::call, 1); // Stack: ns, setter
    } else {
      lua_pushstring(L, name); // Stack: ns, ps, name
      lua_pushcclosure_x(
        L, &detail::read_only_error, 1); // Stack: ns, function
    }
  ...
  template <class TG, class TS = TG>
  Namespace &
  addProperty(const char *name, TG (*get)(), void (*set)(TS) = nullptr) {
    if (m_stackSize == 1) {
      throw_or_assert<std::logic_error>(
        "addProperty() called on global namespace");
    ...
    lua_pushlightuserdata(
      L, reinterpret_cast<void *>(get)); // Stack: ns, function ptr
    lua_pushcclosure_x(
      L, &detail::invoke_proxy_function<TG (*)()>, 1); // Stack: ns, getter
    detail::add_property_getter(L, name, -2);

    if (set != nullptr) {
      lua_pushlightuserdata(
        L, reinterpret_cast<void *>(set)); // Stack: ns, function ptr
      lua_pushcclosure_x(
        L, &detail::invoke_proxy_function<void (*)(TS)>, 1);
    } else {
  ...
  Namespace &addProperty(
    const char *name, TG (*get)() noexcept,
    void (*set)(TS) noexcept = nullptr) {
    if (m_stackSize == 1) {
      throw_or_assert<std::logic_error>(
        "addProperty() called on global namespace");
    ...
    lua_pushlightuserdata(
      L, reinterpret_cast<void *>(get)); // Stack: ns, function ptr
    lua_pushcclosure_x(
      L, &detail::invoke_proxy_function<TG (*)() noexcept>, 1); // Stack: ns, getter
    detail::add_property_getter(L, name, -2);
    ...
    if (set != nullptr) {
      lua_pushlightuserdata(
        L, reinterpret_cast<void *>(set)); // Stack: ns, function ptr
      lua_pushcclosure_x(
        L, &detail::invoke_proxy_function<void (*)(TS) noexcept>, 1);
    } else {
      lua_pushstring(L, name);
      lua_pushcclosure_x(L, &detail::read_only_error, 1);
    }
  ...
  Namespace &addProperty(const char *name, Getter get) {
    ...
    if constexpr (std::is_enum_v<Getter>) {
      ...
      auto enumGet = [get = std::move(get)] {
        return static_cast<U>(get);
      };

      using GetType = decltype(enumGet);
      lua_newuserdata_aligned<GetType>(
        L, std::move(enumGet)); // Stack: ns, function userdata (ud)
      lua_pushcclosure_x(
        L, &detail::invoke_proxy_functor<GetType>, 1); // Stack: ns, ud, getter
    } else {
      using GetType = decltype(get);
      lua_newuserdata_aligned<GetType>(
        L, std::move(get)); // Stack: ns, function userdata (ud)
      lua_pushcclosure_x(
        L, &detail::invoke_proxy_functor<GetType>, 1); // Stack: ns, ud, getter
    }
    ...
    lua_pushcclosure_x(
      L, &detail::read_only_error, 1); // Stack: ns, name, function
    detail::add_property_setter(L, name, -2); // Stack: ns
  ...
  Namespace &addProperty(const char *name, Getter get, Setter set) {
    ...
    lua_newuserdata_aligned<SetType>(
      L, std::move(set)); // Stack: ns, function userdata (ud)
    lua_pushcclosure_x(
      L, &detail::invoke_proxy_functor<SetType>, 1); // Stack: ns, ud, getter
  ...
  template <class... Functions>
  auto
  addFunction(const char *name, Functions... functions) -> std::enable_if_t<
    (detail::is_callable_v<Functions> && ...) && (sizeof...(Functions) > 0),
    Namespace &> {
    assert(name != nullptr);
    assert(lua_istable(L, -1)); // Stack: namespace table (ns)
  ...
  /**
     ...
     *
     * Call deriveClass() only once. To continue registrations for the class
     * later, use beginClass().
     *
     * @param name The class name.
     *
     * @returns A class registration object.
  ...
  Class<Derived> deriveClass(const char *name) {
    ...
    return Class<Derived>(
      name, *this, detail::getStaticRegistryKey<Base>());
  }
};
 ...
 * @brief Retrieve the namespace on top of the stack.
 *
 * You should have a table on top of the stack before calling this function. It
 * will then use the table there as destination for registrations.
 *
 * @param L A Lua state.
 *
 * @returns A namespace registration object.
 ...
 *
 * This is a backward compatibility mitigation for lua 5.1 not supporting
 * LUA_RIDX_MAINTHREAD.
 *
