...
class Iterator {
  ...
  explicit Iterator(const LuaRef &table, bool isEnd = false)
    : m_L(table.state()), m_table(table), m_key(table.state()) // m_key is nil
      ,
  ...
  /**
     ...
     * @return A Lua state.
     */
  lua_State *state() const noexcept { return m_L; }
  ...
  /**
     ...
     *
     * @return True if iterators point to the same entry of the same table,
     * false otherwise.
     */
  ...
  /**
     ...
     *
     * @return True if there are no more table entries to iterate, false
     * otherwise.
     */
  bool isNil() const noexcept { return m_key.isNil(); }
