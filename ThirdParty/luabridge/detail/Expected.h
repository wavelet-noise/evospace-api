...
T *construct_at(T *ptr, Args &&...args) noexcept(
    std::is_nothrow_constructible<T, Args...>::value
) {
    return static_cast<T *>(::new (
...
struct is_swappable_with_impl<
    T, U, std::void_t<decltype(swap(std::declval<T>(), std::declval<U>()))>>
    : std::true_type {};
...
struct is_swappable_with
    : std::conjunction<
          is_swappable_with_impl<
              std::add_lvalue_reference_t<T>, std::add_lvalue_reference_t<U>>,
          is_swappable_with_impl<
...
struct is_nothrow_swappable_with
    : std::conjunction<
          is_swappable_with<T, U>, is_nothrow_swappable_with_impl<T, U>>::type {
};

template <class T>
struct is_nothrow_swappable
    : std::is_nothrow_swappable_with<
          std::add_lvalue_reference_t<T>, std::add_lvalue_reference_t<T>> {};
} // namespace detail
...
namespace detail {
template <
    class T, class E, bool = std::is_default_constructible_v<T>,
    bool = (std::is_void_v<T> || std::is_trivial_v<T>)&&std::is_trivial_v<E>>
union expected_storage {
  public:
    template <
        class U = T,
        class = std::enable_if_t<std::is_default_constructible_v<U>>>
    constexpr expected_storage() noexcept : value_() {}

    template <class... Args>
    constexpr explicit expected_storage(
...
template <class E> union expected_storage<void, E, true, true> {
  public:
    constexpr expected_storage() noexcept : dummy_(0) {}
...
template <class T, class E> union expected_storage<T, E, true, false> {
  public:
...
template <class T, class E> union expected_storage<T, E, false, false> {
  public:
    constexpr explicit expected_storage() noexcept : dummy_(0) {}
...
template <class E> union expected_storage<void, E, true, false> {
  public:
    constexpr expected_storage() noexcept : dummy_(0) {}
...
class ExpectedBaseTrivial {
  ...
  protected:
    ...
    constexpr ExpectedBaseTrivial() noexcept : valid_(true) {}

    template <class... Args>
    constexpr ExpectedBaseTrivial(std::in_place_t, Args &&...args) noexcept
        : storage_(std::in_place, std::forward<Args>(args)...), valid_(true) {}

    template <class... Args>
    constexpr ExpectedBaseTrivial(UnexpectType, Args &&...args) noexcept
        : storage_(unexpect, std::forward<Args>(args)...), valid_(false) {}
    ...
    constexpr T *valuePtr() noexcept { return std::addressof(value()); }

    constexpr const E *errorPtr() const noexcept {
        return std::addressof(error());
    }

    constexpr E *errorPtr() noexcept { return std::addressof(error()); }
...
class ExpectedBaseNonTrivial {
  ...
  protected:
    ...
    constexpr ExpectedBaseNonTrivial(
    ...
    constexpr ExpectedBaseNonTrivial(
        std::in_place_t, Args &&...args
                   ...
                   is_nothrow_constructible_v<
                       storage_type, std::in_place_t, Args...>)
        : storage_(std::in_place, std::forward<Args>(args)...), valid_(true) {}
    ...
    constexpr ExpectedBaseNonTrivial(
        UnexpectType, Args &&...args
                   ...
                   is_nothrow_constructible_v<
                       storage_type, UnexpectType, Args...>)
        : storage_(unexpect, std::forward<Args>(args)...), valid_(false) {}
    ...
    ~ExpectedBaseNonTrivial(
    ) noexcept(noexcept(std::declval<this_type>().destroy())) {
        destroy();
    }
...
template <class T, class E> class ExpectedBaseNonTrivial<T, E, false, false> {
...
template <class E> class Unexpected {
    static_assert(
        !std::is_reference_v<E> && !std::is_void_v<E>,
        "Unexpected type can't be a reference or void"
    );
...
template <class E>
constexpr bool
operator==(const Unexpected<E> &lhs, const Unexpected<E> &rhs) noexcept {
...
template <class E>
constexpr inline Unexpected<std::decay_t<E>> makeUnexpected(E &&error
) noexcept(std::is_nothrow_constructible_v<Unexpected<std::decay_t<E>>, E>) {
...
template <class E> class BadExpectedAccess : public BadExpectedAccess<void> {
  public:
    explicit BadExpectedAccess(E error
...
template <class T, class E>
class Expected : public detail::ExpectedBase<
                     T, E, std::is_copy_constructible_v<T>,
    ...
    using base_type = detail::ExpectedBase<
        T, E, std::is_copy_constructible_v<T>, std::is_move_constructible_v<T>>;
  ...
  public:
    ...
    ) noexcept(std::is_nothrow_copy_constructible_v<base_type>) = default;

    constexpr Expected(Expected &&other
    ) noexcept(std::is_nothrow_move_constructible_v<base_type>) = default;

    template <class U, class G> Expected(const Expected<U, G> &other) {
        if (other.hasValue()) {
    ...
    template <class U, class G> Expected(Expected<U, G> &&other) {
        if (other.hasValue()) {
    ...
    template <
        ...
        std::enable_if_t<
            !std::is_void_v<T> && std::is_constructible_v<T, U &&> &&
                ...
                !is_unexpected<std::decay_t<U>>::value,
            int> = 0>
    constexpr Expected(U &&value
    ) noexcept(std::is_nothrow_constructible_v<base_type, std::in_place_t, U>)
    ...
    constexpr explicit Expected(
        std::in_place_t, std::initializer_list<U> ilist, Args &&...args
                   ...
                   is_nothrow_constructible_v<
                       base_type, std::in_place_t, std::initializer_list<U>,
                       Args...>)
    ...
    constexpr Expected(const Unexpected<G> &u
                   ...
                   is_nothrow_constructible_v<
                       base_type, UnexpectType, const G &>)
        : base_type(unexpect, u.value()) {}

    template <class G = E>
    constexpr Expected(Unexpected<G> &&u
    ) noexcept(std::is_nothrow_constructible_v<base_type, UnexpectType, G &&>)
    ...
    constexpr explicit Expected(
        UnexpectType, Args &&...args
                   ...
                   is_nothrow_constructible_v<base_type, UnexpectType, Args...>)
        : base_type(unexpect, std::forward<Args>(args)...) {}

    template <class U, class... Args>
    constexpr explicit Expected(
        UnexpectType, std::initializer_list<U> ilist, Args &&...args
    ) noexcept(std::
                   is_nothrow_constructible_v<
                       base_type, UnexpectType, std::initializer_list<U>,
                       Args...>)
    ...
    template <class G = E> Expected &operator=(const Unexpected<G> &u) {
        assign(unexpect, u.value());
        return *this;
    }

    template <class G = E> Expected &operator=(Unexpected<G> &&u) {
    ...
    template <class... Args>
    T &emplace(Args &&...args
    ) noexcept(noexcept(std::declval<this_type>()
                            .assign(std::in_place, std::forward<Args>(args)...))
    ) {
    ...
    T &emplace(std::initializer_list<U> ilist, Args &&...args) noexcept(
        noexcept(std::declval<this_type>()
                     .assign(std::in_place, ilist, std::forward<Args>(args)...))
    ) {
        return assign(std::in_place, ilist, std::forward<Args>(args)...);
    }

    void swap(Expected &other
    ) noexcept(detail::is_nothrow_swappable<value_type>::value
    ...
    constexpr const T *operator->() const { return base_type::valuePtr(); }

    constexpr T *operator->() { return base_type::valuePtr(); }
    ...
    template <class U> T valueOr(U &&defaultValue) && {
...
template <class E>
class Expected<void, E> : public detail::ExpectedBase<
                              void, E, std::is_copy_constructible_v<E>,
  ...
  public:
    ...
    template <class G> Expected(const Expected<void, G> &other) {
        if (other.hasValue()) {
            this->valid_ = true;
        } else {
            this->construct(unexpect, other.error());
    ...
    template <class G> Expected(Expected<void, G> &&other) {
        if (other.hasValue()) {
            this->valid_ = true;
        } else {
            this->construct(unexpect, std::move(other.error()));
    ...
    void swap(Expected &other
    ) noexcept(detail::is_nothrow_swappable<error_type>::value) {
        using std::swap;
...
template <class E>
constexpr bool
operator==(const Expected<void, E> &lhs, const Expected<void, E> &rhs) {
    return (lhs && rhs) ? true
                        : ((!lhs && !rhs) ? lhs.error() == rhs.error() : false);
}
