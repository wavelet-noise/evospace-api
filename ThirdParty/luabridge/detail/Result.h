...
struct Result {
    ...
    Result(std::error_code ec) noexcept : m_ec(ec) {}
    ...
    operator std::error_code() const noexcept { return m_ec; }

    std::string message() const { return m_ec.message(); }
 ...
 */
template <class T> struct TypeResult {
    TypeResult() = default;
    ...
    TypeResult(std::error_code ec) noexcept : m_value(makeUnexpected(ec)) {}
    ...
    explicit operator bool() const { return m_value.hasValue(); }
    ...
    std::error_code error() const { return m_value.error(); }

    operator std::error_code() const { return m_value.error(); }

    std::string message() const { return m_value.error().message(); }
