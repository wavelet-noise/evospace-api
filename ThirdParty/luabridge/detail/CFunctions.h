...
namespace luabridge { namespace detail {

//=================================================================================================
/**
 * @brief Make argument lists extracting them from the lua state, starting at a
 * stack index.
 *
...
auto make_arguments_list_impl(lua_State *L, std::index_sequence<Indices...>) {
    return tupleize(
        unwrap_argument_or_error<std::tuple_element_t<Indices, ArgsPack>>(
            L, Start + Indices
        )...
    );
}
...
auto make_arguments_list(lua_State *L) {
    return make_arguments_list_impl<ArgsPack, Start>(
        L, std::make_index_sequence<std::tuple_size_v<ArgsPack>>()
    );
}

//=================================================================================================
/**
 * @brief Helpers for iterating through tuple arguments, pushing each argument
 * to the lua stack.
 */
template <std::size_t Index = 0, class... Types>
auto push_arguments(lua_State *, std::tuple<Types...>) -> std::enable_if_t<
    Index == sizeof...(Types), std::tuple<Result, std::size_t>> {
...
//=================================================================================================
/**
 * @brief Helpers for iterating through tuple arguments, popping each argument
 * from the lua stack.
 */
...
//=================================================================================================
/**
 * @brief __index metamethod for a namespace or class static and non-static
 * members.
 *
 * Retrieves functions from metatables and properties from propget tables. Looks
 * through the class hierarchy if inheritance is present.
 */
inline int index_metamethod(lua_State *L) {
    ...
    ); // Stack (further not shown): table | userdata, name
    ...
    for (;;) {
        ...
        // It may mean that the field may be in const table and it's constness
        // violation. Don't check that, just return nil
        ...
        if (lua_isnil(L, -1)) // Stack: mt, nil
        {
            lua_pop(L, 1); // Stack: mt
            lua_rawgetp(
                L, -1, getIndexFallbackKey()
            );                 // Stack: mt, ifb (may be nil)
...
inline int newindex_metamethod(lua_State *L, bool pushSelf) {
    ...
    assert(
        lua_istable(L, 1) || lua_isuserdata(L, 1)
    ); // Stack (further not shown): table | userdata, name, new value
    ...
    for (;;) {
        ...
        if (lua_isnil(L, -1)) // Stack: mt, nil
        {
            lua_pop(L, 1); // Stack: mt
            lua_rawgetp(
                L, -1, getNewIndexFallbackKey()
            ); // Stack: mt, nifb (may be nil)
            if (lua_iscfunction(L, -1)) {
 ...
 */
template <class C> static int gc_metamethod(lua_State *L) {
...
template <class T, class C = void> struct property_getter;

/**
 * @brief lua_CFunction to get a variable.
 *
 * This is used for global variables or class static data members. The pointer
 * to the data is in the first upvalue.
 */
template <class T> struct property_getter<T, void> {
 ...
 *
 * The pointer-to-member is in the first upvalue. The class userdata object is
 * at the top of the Lua stack.
 */
template <class T, class C> struct property_getter {
    static int call(lua_State *L) {
...
/**
 * @brief Helper function to push a property getter on a table at a specific
 * index.
 */
inline void
add_property_getter(lua_State *L, const char *name, int tableIndex) {
#if LUABRIDGE_SAFE_STACK_CHECKS
    ...
    lua_rawgetp(
        L, tableIndex, getPropgetKey()
    );                        // Stack: getter, propget table (pg)
...
template <class T, class C = void> struct property_setter;
 ...
 * to the data is in the first upvalue.
 */
template <class T> struct property_setter<T, void> {
    static int call(lua_State *L) {
        assert(lua_islightuserdata(L, lua_upvalueindex(1)));
 ...
 * at the top of the Lua stack.
 */
template <class T, class C> struct property_setter {
    static int call(lua_State *L) {
...
/**
 * @brief Helper function to push a property setter on a table at a specific
 * index.
 */
inline void
add_property_setter(lua_State *L, const char *name, int tableIndex) {
#if LUABRIDGE_SAFE_STACK_CHECKS
    luaL_checkstack(L, 2, detail::error_lua_stack_overflow);
#endif
    ...
    lua_rawgetp(
        L, tableIndex, getPropsetKey()
    );                        // Stack: setter, propset table (ps)
...
struct function {
    template <class F> static int call(lua_State *L, F func) {
        Result result;

#if LUABRIDGE_HAS_EXCEPTIONS
        try {
#endif
            result = Stack<ReturnType>::push(
    ...
    template <class T, class F> static int call(lua_State *L, T *ptr, F func) {
        Result result;

#if LUABRIDGE_HAS_EXCEPTIONS
        try {
#endif
            auto f = [ptr, func](auto &&...args) -> ReturnType {
                return (ptr->*func)(std::forward<decltype(args)>(args)...);
            };

            result = Stack<ReturnType>::push(
                L, std::apply(f, make_arguments_list<ArgsPack, Start>(L))
            );
...
struct function<void, ArgsPack, Start> {
    ...
    template <class T, class F> static int call(lua_State *L, T *ptr, F func) {
        ...
        try {
#endif
            auto f = [ptr, func](auto &&...args) {
                (ptr->*func)(std::forward<decltype(args)>(args)...);
            };
 ...
 *
 * The member function pointer is in the first upvalue. The class userdata
 * object is at the top of the Lua stack.
 */
template <class F, class T> int invoke_member_function(lua_State *L) {
    ...
    const F &func =
        *static_cast<const F *>(lua_touserdata(L, lua_upvalueindex(1)));
    assert(func != nullptr);
...
template <class F, class T> int invoke_const_member_function(lua_State *L) {
    using FnTraits = detail::function_traits<F>;
 ...
 *
 * The member function pointer is in the first upvalue. The object userdata
 ...
 *
 * The proxy function pointer (lightuserdata) is in the first upvalue. The class
 * userdata object is at the top of the Lua stack.
 */
template <class F> int invoke_proxy_function(lua_State *L) {
    using FnTraits = detail::function_traits<F>;
...
//=================================================================================================
/**
 * @brief lua_CFunction to call on a object via functor (lambda wrapped in a
 * std::function).
 *
 * The proxy std::function (lightuserdata) is in the first upvalue. The class
 * userdata object is at the top of the Lua stack.
 */
template <class F> int invoke_proxy_functor(lua_State *L) {
    using FnTraits = detail::function_traits<F>;
 ...
 *
 * The list of overloads is in the first upvalue. The arguments of the function
 * call are at the top of the Lua stack.
 */
template <bool Member> inline int try_overload_functions(lua_State *L) {
    ...
    while (lua_next(L, idx_overloads) != 0) {
        ...
        if (overload_arity >= 0 && overload_arity != effective_args) {
            ...
            lua_pushfstring(
                L,
                "Skipped overload #%d with unmatched arity of %d instead of %d",
                nerrors,
                overload_arity,
                effective_args
            );
        ...
        if (err == LUABRIDGE_LUA_OK) {
            ...
            const int nresults =
                lua_gettop(L) - nargs - 4; // 4: overloads, errors, key, table
            return nresults;
    ...
    lua_pushfstring(
        L, "All %d overloads of %s returned an error:", nerrors, debug.name
    );
...
template <
    class F, class = std::enable_if<
                 is_callable_v<F> && !std::is_pointer_v<F> &&
...
//=================================================================================================
// Lua CFunction
template <class T> void push_member_function(lua_State *L, lua_CFunction fp) {
    lua_pushcfunction_x(L, fp);
}
...
template <class T, class ReturnType, class... Params>
void push_member_function(
    lua_State *L, ReturnType (*fp)(T *, Params...) noexcept
) {
    using FnType = decltype(fp);
...
template <class T, class ReturnType, class... Params>
void push_member_function(
    lua_State *L, ReturnType (*fp)(const T *, Params...)
) {
    using FnType = decltype(fp);
...
template <class T, class ReturnType, class... Params>
void push_member_function(
    lua_State *L, ReturnType (*fp)(const T *, Params...) noexcept
) {
    using FnType = decltype(fp);
...
template <
    class T, class F,
    class = std::enable_if<
        is_callable_v<F> && std::is_object_v<F> && !std::is_pointer_v<F> &&
        !std::is_member_function_pointer_v<F>>>
void push_member_function(lua_State *L, F &&f) {
    static_assert(std::is_same_v<
                  T,
                  remove_cvref_t<std::remove_pointer_t<
                      function_argument_or_void_t<0, F>>>>);
...
template <class T, class U, class ReturnType, class... Params>
void push_member_function(
    lua_State *L, ReturnType (U::*mfp)(Params...) noexcept
) {
    static_assert(std::is_same_v<T, U> || std::is_base_of_v<U, T>);
...
template <class T, class U, class ReturnType, class... Params>
void push_member_function(
    lua_State *L, ReturnType (U::*mfp)(Params...) const noexcept
) {
    static_assert(std::is_same_v<T, U> || std::is_base_of_v<U, T>);
 ...
 *
 * These templates call operator new with the contents of a type/value list
 * passed to the constructor. Two versions of call() are provided. One performs
 * a regular new, the other performs a placement new.
 */
template <class T, class Args> struct constructor;

template <class T> struct constructor<T, void> {
    using empty = std::tuple<>;

    static T *call(const empty &) { return new T; }

    static T *call(void *ptr, const empty &) { return new (ptr) T; }
};
 ...
 */
template <class T> struct placement_constructor {
    template <class F, class Args>
    static T *construct(void *ptr, const F &func, const Args &args) {
        auto alloc = [ptr, &func](auto &&...args) {
            return func(ptr, std::forward<decltype(args)>(args)...);
        };

        return std::apply(alloc, args);
    }

    template <class F> static T *construct(void *ptr, const F &func) {
        return func(ptr);
    }
};
 ...
 */
template <class T> struct external_constructor {
    template <class F, class Args>
    static T *construct(const F &func, const Args &args) {
        auto alloc = [&func](auto &&...args) {
    ...
    template <class F> static T *construct(const F &func) { return func(); }
};
 ...
 */
template <class C, class Args> int constructor_container_proxy(lua_State *L) {
    using T = typename ContainerTraits<C>::Type;

    T *object = detail::constructor<T, Args>::call(
        detail::make_arguments_list<Args, 2>(L)
    );
 ...
 */
template <class T, class Args> int constructor_placement_proxy(lua_State *L) {
    std::error_code ec;
    auto *value = detail::UserdataValue<T>::place(L, ec);
    if (!value)
        luaL_error(L, "%s", ec.message().c_str());

    detail::constructor<T, Args>::call(
        value->getObject(), detail::make_arguments_list<Args, 2>(L)
    );
 ...
 */
template <class T, class F> struct constructor_forwarder {
    explicit constructor_forwarder(F f) : m_func(std::move(f)) {}
 ...
 * @brief Constructor forwarder.
 */
template <class T, class Alloc, class Dealloc> struct factory_forwarder {
    explicit factory_forwarder(Alloc alloc, Dealloc dealloc)
        : m_alloc(std::move(alloc)), m_dealloc(std::move(dealloc)) {}

    T *operator()(lua_State *L) {
        using FnTraits = function_traits<Alloc>;
        using FnArgs = typename FnTraits::argument_types;

        T *obj = external_constructor<T>::construct(
            m_alloc, make_arguments_list<FnArgs, 0>(L)
        );
