 ...
 */
  inline int index_metamethod(lua_State *L) {
    ...
    assert(
      lua_istable(L, 1) || lua_isuserdata(L, 1)); // Stack (further not shown): table | userdata, name
    ...
    for (;;) {
      ...
      if (lua_isnil(L, -1)) // Stack: mt, nil
      {
        lua_pop(L, 1); // Stack: mt
        lua_rawgetp(
          L, -1, getIndexFallbackKey()); // Stack: mt, ifb (may be nil)
 ...
 */
  inline int newindex_metamethod(lua_State *L, bool pushSelf) {
    ...
    for (;;) {
      lua_rawgetp(
        L, -1, getPropsetKey()); // Stack: mt, propset table (ps) | nil
      ...
      if (lua_isnil(L, -1)) // Stack: mt, nil
      {
        lua_pop(L, 1); // Stack: mt
        lua_rawgetp(
          L, -1, getNewIndexFallbackKey()); // Stack: mt, nifb (may be nil)
        if (lua_iscfunction(L, -1)) {
 ...
 */
  ...
  add_property_getter(lua_State *L, const char *name, int tableIndex) {
    ...
    lua_rawgetp(
      L, tableIndex, getPropgetKey()); // Stack: getter, propget table (pg)
 ...
 */
  ...
  add_property_setter(lua_State *L, const char *name, int tableIndex) {
    ...
    lua_rawgetp(
      L, tableIndex, getPropsetKey()); // Stack: setter, propset table (ps)
 ...
 */
  ...
  struct external_constructor {
    ...
    template <class F>
    static T *construct(const F &func) { return func(); }
  };
