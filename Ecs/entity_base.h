...
struct EntityFactory {
    ...
    using FactoryType =
        evo::TemplateFactory<std::string, ecs::Entity, cmpNameFast>;
...
template <typename Ty_> struct EcsComponentBase {
    static auto GetPrototypeLambda() {
        return [](ecs::Entity *e, void *proto) {
            e->assign_prototype<Ty_>(*static_cast<Ty_ *>(proto));
        };
    }
    ...
    static auto GetLuaLambda() {
        return [](lua_State *state, void *data) {
            std::error_code er;
            luabridge::push(state, static_cast<Ty_ *>(data), er);
        };
    }
