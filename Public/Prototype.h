...
#define EVO_LUA_CODEGEN_DB(type, name)                                                                                      \
  ...
  static void lua_reg_internal(lua_State *L) {                                                                              \
    ...
    luabridge::getGlobalNamespace(L)                                                                                        \
      ...
      .addStaticFunction(                                                                                                   \
        "find", +[](std::string_view name) { return FindObject<type>(ANY_PACKAGE, UTF8_TO_TCHAR(name.data())); })           \
...
#define EVO_LUA_CODEGEN_DB_EX(type)                                                                                            \
  ...
  static void lua_reg_internal(lua_State *L) {                                                                                 \
    ...
    luabridge::getGlobalNamespace(L)                                                                                           \
      ...
      .addStaticFunction(                                                                                                      \
        "find", +[](std::string_view name) { return FindObject<U##type>(ANY_PACKAGE, UTF8_TO_TCHAR(name.data())); })           \
